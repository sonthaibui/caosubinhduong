...
class TestSaleAdvancePayment(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.currency_euro = cls.env["res.currency"].search(
            [("name", "=", "EUR")])
    ...
    def test_01_sale_advance_payment(self):
        self.assertEqual(
            self.sale_order_1.remain_amount,
            3600,
        )
        self.assertEqual(
            self.sale_order_1.remain_amount,
        ...
        self.assertEqual(self.sale_order_1.remain_amount, 3480)
        ...
        self.assertEqual(self.sale_order_1.remain_amount, 3280)
        ...
        self.assertEqual(self.sale_order_1.remain_amount, 2980)
        ...
        self.assertEqual(self.sale_order_1.remain_amount, 2580)
    ...
    def test_02_residual_amount_with_invoice(self):
        ...
        self.assertEqual(self.sale_order_1.remain_amount, 3400)
        ...
        self.assertEqual(self.sale_order_1.remain_amount, 2200)

        # Reconciling the pre-payment should not affect remain_amount in SO.
        (
            liquidity_lines,
            counterpart_lines,
            writeoff_lines,
    ...
    def test_03_residual_amount_big_pre_payment(self):
        ...
        pre_payment = self.sale_order_1.account_payment_ids
        self.assertEqual(len(pre_payment), 1)
        self.assertEqual(self.sale_order_1.remain_amount, 1600)
        ...
        invoice.invoice_date = fields.Date.today()
        invoice.action_post()
        self.assertEqual(invoice.remain_amount, 1200)
        active_ids = invoice.ids
        self.env["account.payment.register"].with_context(
            active_model="account.move", active_ids=active_ids
        ).create(
        ...
        self.assertEqual(invoice.remain_amount, 900)
        self.assertEqual(self.sale_order_1.remain_amount, 1300)

        # Partially reconciling the pre-payment should not affect remain_amount in SO.
        (
            liquidity_lines,
            counterpart_lines,
            writeoff_lines,
        ...
        self.sale_order_1.invalidate_cache()
        self.assertEqual(self.sale_order_1.remain_amount, 1300)
        self.assertEqual(invoice.remain_amount, 0)

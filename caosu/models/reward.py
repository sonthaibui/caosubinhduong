...
class Reward(models.Model):
    ...
    def _compute_tongtien(self):
        for rec in self:
            ...
            if self.env['reward'].search([('employee_id','=', rec.employee_id.name), ('namkt','=',rec.namkt )]):                
                rs = self.env['reward'].search([('employee_id','=', rec.employee_id.name), ('namkt','=',rec.namkt )])             
                for r in rs:                                                
                    rec.tongtien_luyke += r.tongtien

    @api.depends('thang', 'nam', 'tongtien')

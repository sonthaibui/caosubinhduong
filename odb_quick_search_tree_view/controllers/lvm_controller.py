...
class LvmController(DataSet, http.Controller):
    ...
    def ks_prepare_lvm_list_data(self, original_list_data, model, ks_list_view_id):
        ...
        if ks_list_view_id:
            ...
            if list_view_data['ks_lvm_user_data']['ks_lvm_user_table_result']['quick_search_fields_data']:
    ...
    def update_list_view_data(self, ks_table_data, quick_search_fields_data, ks_fetch_options):
        for ks_table in ks_table_data:
            request.env['user.specific'].browse(ks_table.get('id')).write(ks_table)

        for quick_search_field in quick_search_fields_data:
            request.env['user.fields'].browse(quick_search_field.get('id')).write(quick_search_field)
    ...
    def create_list_view_data(self, ks_model, quick_search_editable, ks_view_id, ks_table_width_per, quick_search_fields_data):
        list_view_record = request.env['user.specific'].create({
            'model_name': ks_model,
            'user_id': request.env.uid,
            'ks_action_id': ks_view_id,
            'ks_table_width': ks_table_width_per,
            'quick_search_editable': quick_search_editable,
        })

        for rec in quick_search_fields_data.values():
    ...
    def check_fields(self, table_id, fields_list, quick_search_field_list):
        for r_field in filter(lambda x: x not in [x for x in fields_list.keys()], [x['field_name'] for x in quick_search_field_list.values()]):
            field_rec = quick_search_field_list.pop(r_field)
            request.env['user.fields'].browse(field_rec.get('id', 0)).sudo().unlink()

        for field in filter(lambda x: not quick_search_field_list.get(x, False), fields_list.keys()):
            quick_search_field_list[field] = val = {
                "quick_search_columns_name": fields_list[field]['string'],
                "ksShowField": False,
                "field_name": field,
                "ks_width": 0,
                "field_order": len(quick_search_field_list)
            }
            val.update({'fields_list': table_id})
            rec_id = request.env['user.fields'].create(val)
            quick_search_field_list[field]['id'] = rec_id.id
    ...
    def ks_process_arch(self, list_view_data, fields_list):
        ...
        for field_node in node_children:
            ...
            if field_node.get("name") in quick_search_field_list:
                if field_node.get("name") and not quick_search_field_list[field_node.get("name")]['ksShowField'] and field_node.tag != "field":
                    node.remove(field_node)
            else:
                quick_search_field_list[field_node.get("name")] = val = {
                    "quick_search_columns_name": field_node.attrib['name'],
                    "ksShowField": False,
                ...
                rec_id = request.env['user.fields'].create(val)
                quick_search_field_list[field_node.get("name")]['id'] = rec_id.id

        # Only showing selected fields to visible
        for field_name in [x['field_name'] for x in quick_search_field_list.values() if x['ksShowField']]:
            if list(filter(lambda x: x.get('name') == field_name,node_children)):
                for field_node in list(filter(lambda x: x.get('name') == field_name, node_children)):
                    field_node.set('invisible', '0')
                    field_node.set('string', quick_search_field_list[field_name]['quick_search_columns_name'])
                ...
                field_node = etree.Element('field', attrib={'name': field_name, 'invisible': '0',
                                                            'string': quick_search_field_list[field_name]['quick_search_columns_name']})
        ...
        sorted_node_fields = sorted(node_children, key=lambda x: quick_search_field_list[x.get('name')]['field_order'])
        for field_node in node_children:
            node.remove(field_node)

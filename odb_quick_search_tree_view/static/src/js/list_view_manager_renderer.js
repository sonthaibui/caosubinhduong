odoo.define('odb_quick_search_tree_view.renderer', function (require) {
    ...
    ListRenderer.include({
        ...
        ks_on_date_filter_change: function(ks_widget_key){
            ...
            if(ks_date_widget.getValue()){
                this.trigger_up("update_advance_search_renderer",{
                ...
                if(!(target.parentElement.id.indexOf("_lvm_end_date") > 0)) {
                    $('#'+target.parentElement.id).parent().addClass("date_main");
                    $($('#'+target.parentElement.id).parent().parent().children()[1]).addClass("date_inner");
        ...
        init: function (parent, state, params) {
            ...
            self.quick_search_fields_data = self.ks_user_table_result.quick_search_fields_data ? self.ks_user_table_result.quick_search_fields_data : self.ksComputeFieldData(params.arch, state.fields);
            self.quick_search_field_domain_dict={}
            self.ks_key_fields=[]
            self.quick_search_field_domain_list=[]
            self.ksDomain=null;
            ...
            self.is_quick_search_editable_on = false;
        ...
        _getNumberOfCols: function () {
            if(!this.ks_lvm_mode) return this._super.apply(this, arguments);
            var quick_search_columns = this._super();
            quick_search_columns +=1;

            return quick_search_columns;
        },
        ...
        _renderHeader: function (isGrouped) {
            ...
            self.ks_allow_search = true;
            self.quick_search_field_popup = {};
            if(self.ksDomain!=null) {
                for(var i=0; i < self.ksDomain.length; i++) {
                    if(!(self.ksDomain[i] === '|')) {
                        if(self.quick_search_field_popup[self.ksDomain[i][0]] === undefined) {
                            self.quick_search_field_popup[self.ksDomain[i][0]]=[self.ksDomain[i][2]]
                        } else {
                            self.quick_search_field_popup[self.ksDomain[i][0]].push(self.ksDomain[i][2])
                        }
                    }
                }
            }
...
//                        $(table).find(".o_resize").css("cursor","none")
                        ...
                        for(var i = 2 - ks_iter;i < self.$el.find("thead .bg-primary th").length; i++) {
                            quick_search_fields_name.push(self.$el.find("thead .bg-primary th")[i].innerText);
                        }

                        var quick_search_fields_length = quick_search_fields_name.length;
                        ...
                        if(this.$el.parents().find(".o_modal_header").length === 0) {
                            ...
                            if(session.ks_header_color) {
                                if(!(this.getParent().$el.hasClass("o_field_one2many")|| this.getParent().$el.hasClass("o_field_many2many"))) {
                                    ...
                                    var ks_header_search_children = self.$el.find(".quick_search_advance_row")
...
//            return this._super.apply(this, arguments);
        ...
        */
        ...
        ksComputeFieldData: function(arch,fields){
            var quick_search_field_list = {};
            //            Making Field List
            var sort_counter = arch.children.length;
            _.map(fields, function(x,y){
                    if (y !== "activity_exception_decoration"){
                        quick_search_field_list[y]= {
                            quick_search_columns_name:x.string,
                            ksShowField: false,
                            field_name: y,
                            ks_width: 0,
                            field_order: sort_counter
                        }
                    }
            })
//            Assigning visible/invisible from arch
            ...
            _.map(arch.children,function(x){
                ...
                if(quick_search_field_list.hasOwnProperty(x.attrs.name)){
                    _.extend(quick_search_field_list[x.attrs.name],{
                        ksShowField: !invis,
                        field_order: sort_counter,
                        quick_search_columns_name: x.attrs.string || quick_search_field_list[x.attrs.name].quick_search_columns_name
                    });
                    sort_counter+=1;
                }else if (x.attrs.name){
                    quick_search_field_list[x.attrs.name] = {
                        quick_search_columns_name: x.attrs.string || "Undefined",
                        ksShowField: !invis,
            ...
            return quick_search_field_list;
        },
                ...
                function sticky(){
                    self.$el.find(".table.o_list_table").each(function () {
                        ...
                        if(!self.state.groupedBy.length) {
                            self.$el.find('table.o_list_table').ResizableColumns({
                                update: function($col_target) {
                                           self.trigger_up('search_on_list_header_resize', {
                                                ks_width: $col_target.parent().innerWidth(),
                                                quick_search_field_name: $col_target.parent().data().name,
                                            });
                                        }
                            });
                        }
...
//                table.style.tableLayout = 'fixed';
        ...
        _onToggleOptionalColumn: function (ev) {
            ...
            var self = this;
            if($(ev.target).parents().hasClass("quick_search_columns")) {
        ...
        _onKeyDown: function (ev) {
            if(!this.ks_lvm_mode) return this._super.apply(this, arguments);
            if($(ev.currentTarget).hasClass("quick_search_advance_row"))
                return;
            else
                this._super.apply(this,arguments);
        },
        ...
        on_attach_callback: function () {
            ...
            if($(".modal").length === 0) {
                    ...
                    if(self.ks_mode_count === 0) {
                        if(self.userMode.length === 1) {
                            if(self.userMode[0].editable === "top") {
                                ...
                                self.is_quick_search_editable_on = true;
                                self.editable = "top";
                ...
                if(self.is_quick_search_editable_on){
                    self.editable = ''
                }
            }
...
//                self.ks_set_width_according_to_result();
        ...
        ks_fetch_autocomplete_data: function (field,type,value,search_one2many_relation) {
            var self = this;

            return self._rpc({
                model: 'user.mode',
                method: 'get_autocomplete_values',
                args: [self.state.model,field,type,value,search_one2many_relation],
            })
        },
        ...
        ks_textBox: function(node) {
            ...
            if(node.tag === "field") {
                if(self.state.fields[node.attrs.name].store === true && !(self.state.fields[node.attrs.name].type === "one2many" || self.state.fields[node.attrs.name].type === "many2many")) {
                    ...
                    var quick_search_fields = self.state.fields[ks_name];
                    var ks_selection_values = []
                    var ks_description;
                    var quick_search_field_type;
                    var $ks_from;
                    var quick_search_field_identity;
                    var ks_identity_flag=false;
                    var quick_search_field_id=ks_name;
                    var ks_is_hide = true;
                    ...
                    if(quick_search_fields) {
                        quick_search_field_type = self.state.fields[ks_name].type;

                        if(quick_search_field_type === "selection") {
                            ks_selection_values = self.state.fields[ks_name].selection;
                        }
                        if (ks_description === undefined) {
                            ks_description = node.attrs.string || quick_search_fields.string;
                        }
                    }

                    var $th = $('<th>').addClass("quick_search_advance_row ");
                    if(quick_search_field_type === "date" || quick_search_field_type === "datetime"){
                        if(self.ks_call_flag > 1){
                            $th.addClass("fix_width");
                        }
                    }
                    ...
                    if(quick_search_field_type === "date" || quick_search_field_type === "datetime"){
                        ...
                        if(self.ks_call_flag == 2 && ks_identity_flag == false) {
                               quick_search_field_id = ks_name+"_lvm_end_date"
                               quick_search_field_identity = quick_search_field_id+" lvm_end_date"
                        } else {
                               quick_search_field_id = ks_name+"_lvm_start_date"
                               quick_search_field_identity = quick_search_field_id+" lvm_start_date"
                        }
                    }

                    var $input =$(QWeb.render("ks_list_view_advance_search", {
                        ks_id : quick_search_field_id,
                        ks_description : ks_description,
                        ks_type:quick_search_field_type,
                        quick_search_field_identifier : quick_search_field_identity,
                        ks_selection: ks_selection_values
                    }));

                    if((quick_search_field_type==="date" || quick_search_field_type==="datetime" ) && (self.ks_call_flag==2 && ks_identity_flag == false)) {
                        if(self.state.domain.length === 0){
                            $input.addClass("d-none");
                            $th.addClass("date_inner");
                        }

                        if(!(self.state.domain.length === 0 )) {
                            if(Object.values(self.quick_search_field_popup) !== undefined){
                                for(var ks_hide = 0; ks_hide < Object.keys(self.quick_search_field_popup).length; ks_hide++) {
                                    if((Object.keys(self.quick_search_field_popup)[ks_hide] === ks_name)){
                                            ks_is_hide=false
                                            break
                                    }
                                }
                    ...
                    if(self.ksDomain != null && self.ksDomain.length) {
                        if(self.ksDomain[self.ksDomain.length-1] === self.state.domain[self.state.domain.length-1]) {
                            if(quick_search_field_type === "date" || quick_search_field_type === "datetime") {
                                for(var ks_add_span = 0; ks_add_span < Object.keys(self.quick_search_field_popup).length; ks_add_span++) {
                                    if(Object.keys(self.quick_search_field_popup)[ks_add_span] === ks_name) {
                                        for(var ks_add_span_inner = 0; ks_add_span_inner < Object.values(self.quick_search_field_popup)[ks_add_span].length-1; ks_add_span_inner++) {

                                            var $div = $('<div>').addClass("inner_search")
                                            $div.attr('id',ks_name+'_value'+ks_add_span_inner)
                                            var $span = $('<span>');
                                            if(quick_search_field_type === "datetime") {
                                              $span = $span.addClass("quick_search_date_chip_ellipsis");
                                            }
                                            ...
                                            if(self.ks_call_flag == 2 && ks_identity_flag==false) {
                                                $span.text(Object.values(self.quick_search_field_popup)[ks_add_span][1])
                                                $span.attr("title",Object.values(self.quick_search_field_popup)[ks_add_span][1]);
                                                $input.prepend($div);
                                                $input.find("#"+Object.keys(self.quick_search_field_popup)[ks_add_span]+"_value"+ks_add_span_inner).prepend($i);
                                                $input.find("#"+Object.keys(self.quick_search_field_popup)[ks_add_span]+"_value"+ks_add_span_inner).prepend($span)
                                            } else {
                                                $input.addClass("date_main");
                                                $span.text(Object.values(self.quick_search_field_popup)[ks_add_span][0]);
                                                $span.attr("title",Object.values(self.quick_search_field_popup)[ks_add_span][0]);
                                                $input.prepend($div);
                                                $input.find("#"+Object.keys(self.quick_search_field_popup)[ks_add_span]+"_value"+ks_add_span_inner).prepend($i);
                                                $input.find("#"+Object.keys(self.quick_search_field_popup)[ks_add_span]+"_value"+ks_add_span_inner).prepend($span);
                                            }
                            ...
                            }else if(quick_search_field_type === "selection") {
                                for(var ks_add_span = 0; ks_add_span < Object.keys(self.quick_search_field_popup).length; ks_add_span++) {
                                    if(Object.keys(self.quick_search_field_popup)[ks_add_span] === ks_name) {
                                        for(var ks_add_span_inner = 0; ks_add_span_inner < Object.values(self.quick_search_field_popup)[ks_add_span].length; ks_add_span_inner++) {
                                            var value;
                                            var $div = $('<div>').addClass("inner_search")
                                            $div.attr('id',ks_name+'_value'+ks_add_span_inner)

                                            var $span = $('<span>').addClass("advance_chip");
                                            $span.attr('id',ks_name+'_ks_span'+ks_add_span_inner)
                                            $span.addClass("advance_chip_ellipsis");

                                            var $i = $('<i>').addClass("fa fa-times")
                                            $i.addClass('ks_remove_popup');

                                            for(var sel=0; sel < ks_selection_values.length; sel++) {
                                                if(ks_selection_values[sel][0] === Object.values(self.quick_search_field_popup)[ks_add_span][ks_add_span_inner]) {
                                   ...
                                   for(var ks_add_span=0; ks_add_span < Object.keys(self.quick_search_field_popup).length; ks_add_span++) {
                                        if(Object.keys(self.quick_search_field_popup)[ks_add_span] === ks_name) {
                                            for(var ks_add_span_inner=0;ks_add_span_inner < Object.values(self.quick_search_field_popup)[ks_add_span].length;ks_add_span_inner++) {
                                                ...
                                                $i.addClass('ks_remove_popup');
                                                if(quick_search_field_type === 'monetary' || quick_search_field_type === 'integer' || quick_search_field_type === 'float') {
                                                    var currency = self.getSession().get_currency(self.ks_list_view_data.currency_id);
                                                    var formatted_value = fieldUtils.format.float(Object.values(self.quick_search_field_popup)[ks_add_span][ks_add_span_inner] || 0, {digits: currency && currency.digits});
                                                    $span.text(formatted_value);
                                                    $span.attr('title',formatted_value);

                                                } else {
                                                    $span.text(Object.values(self.quick_search_field_popup)[ks_add_span][ks_add_span_inner])
                                                    $span.attr('title',Object.values(self.quick_search_field_popup)[ks_add_span][ks_add_span_inner]);
                                                }
                                                 if(!(quick_search_field_type === 'many2one'|| quick_search_field_type === 'many2many' || quick_search_field_type === 'one2many'))
                    ...
                    if(self.ksDomain!=null && self.ksDomain.length) {
                            if(!(self.ksDomain[self.ksDomain.length-1] === self.state.domain[self.state.domain.length-1])) {
                                    delete self.quick_search_field_domain_dict
                                    delete self.ksDomain
                                    self.ksBaseDomain = []
                                    self.quick_search_field_domain_dict = {}
                                    self.ks_key_fields.splice(0,self.ks_key_fields.length)
                                    self.quick_search_field_domain_list.splice(0,self.quick_search_field_domain_list.length)
                            }
                        }
                    ...
                    if(quick_search_field_type === "date" || quick_search_field_type === "datetime") {
                        for(var i = 0; i < self.state.domain.length; i++) {
                            if(quick_search_field_identity.split("_lvm_end_date")[0] === self.state.domain[i][0] || quick_search_field_identity.split("_lvm_start_date")[0] === self.state.domain[i][0]){
                    ...
                    if(ks_widget_flag && quick_search_field_type === "date") {
                        var widget_key = "ksStartdatePickerWidget" + quick_search_field_identity;
                        ...
                        self[widget_key].appendTo($input.find('.custom-control-searchbar-change')).then((function () {
                            self["ksStartdatePickerWidget" + quick_search_field_identity].$el.addClass("quick_search_btn_middle_child o_input");
                            self["ksStartdatePickerWidget" + quick_search_field_identity].$el.find("input").attr("placeholder", "Search");
                        }).bind(this));
                    ...
                    if(ks_widget_flag && quick_search_field_type === "datetime") {
                        var widget_key = "ksStartdatetimePickerWidget" + quick_search_field_identity;
                        self[widget_key] = new (datepicker.DateTimeWidget)(this);
                        self[widget_key].appendTo($input.find('.custom-control-searchbar-change')).then((function () {
                            self["ksStartdatetimePickerWidget" + quick_search_field_identity].$el.addClass("quick_search_btn_middle_child o_input");
                            self["ksStartdatetimePickerWidget" + quick_search_field_identity].$el.find("input").attr("placeholder", "Search");
                        }).bind(this));
                ...
                return $('<th>').addClass("quick_search_advance_row ");;
            }
        },
        ...
        ks_advance_searchbar: function(e) {
            ...
            var self = this;
            var quick_search_field_type = e.currentTarget.dataset.fieldType;
            var quick_search_field_name = e.currentTarget.dataset.quick_search_field_id;
            var search_one2many_relation;
            var ks_input_val = $(e.currentTarget).val();

            if((!(e.keyCode == 8 || e.keyCode ==13)) && $(e.currentTarget).val().length) {

                if(quick_search_field_type === "one2many") {
                    search_one2many_relation = self.state.fields[e.currentTarget.id].relation
                }


                self.ks_fetch_autocomplete_data(e.currentTarget.id,quick_search_field_type,$(e.currentTarget).val(),search_one2many_relation)
                .then(function(ks_auto_Data){

                    self.ks_autocomplete_data_result = ks_auto_Data

                    if(!(quick_search_field_type === "date" || quick_search_field_type === "datetime" || quick_search_field_type === "selection")) {
                        var ks_unique_data = {}
                        ...
                        } else if(quick_search_field_type === 'many2many' || quick_search_field_type === 'many2one'){
                            for(var i = 0; i < self.ks_autocomplete_data_result.length; i++) {
            ...
            if(e.keyCode == 8 && this.ks_allow_search) {
                if(event.target.parentNode.children.length!==1) {
                    this.trigger_up("search_remove_domain",{"event":e})
                    this.ks_allow_search = false;
                }
            }
            if(e.keyCode == 13 && this.ks_allow_search) {
                this.trigger_up("update_advance_search_renderer",{ksFieldName: e.currentTarget.dataset.ksField,KsSearchId:e.currentTarget.id,ksfieldtype:e.currentTarget.dataset.fieldType});
                this.ks_allow_search = false;
            }
        },
        ...
        ks_change_event: function(e) {
            if(e.currentTarget.dataset.fieldType !== "datetime" && e.currentTarget.dataset.fieldType !== 'date') {
                this.trigger_up("update_advance_search_renderer",{ksFieldName: e.currentTarget.dataset.ksField,KsSearchId:e.currentTarget.id,ksfieldtype:e.currentTarget.dataset.fieldType,ksFieldIdentity:e.currentTarget.dataset.fieldIdentity});
            }
        },

        ks_remove_popup_domain: function(e) {

            var div =e.currentTarget.closest('.inner_search')
            this.trigger_up("search_remove_domain",{ksDiv: div,ksfieldtype:e.currentTarget.parentElement.parentElement.children[1].dataset.fieldType,});
        },
        ...
        ks_set_width_according_to_result: function($ks_table) {
            var self = this;
            var quick_search_fields_data_list = self.getParent().quick_search_fields_data || false;
            var ks_table_data = self.getParent().ks_table_data || false;
            if(quick_search_fields_data_list && ks_table_data && ks_table_data.ks_table_width > 0){
                if($ks_table.offset()) {
                    ...
                    var field_width_update_list = _.filter(quick_search_fields_data_list,(x) => x.ks_width>0);
                ...
                for(var i = 0; i < $(".quick_search_advance_row").length; i++) {
                    if(self.$el.find(".quick_search_advance_row")[i]){
                        self.$el.find(".quick_search_advance_row")[i].style.setProperty("width","")
                    }
                }

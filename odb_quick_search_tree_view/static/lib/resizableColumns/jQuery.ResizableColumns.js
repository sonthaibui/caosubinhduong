(function($) {
    $.fn.ResizableColumns = function(option) {
      ...
      var update = option.update || function(){};
        ...
        const _onStartResize = function(e){
            // Class Used to identify changes : 'quick_search_resize', 'quick_search_resizer', 'ks_resizing'
            ...
            $(_thead).find('tr:first-child th:nth-child(' + ($(this).closest('th').index() + 1) + ') .quick_search_resizer').closest('th').addClass('ks_resizing');
            isColResizing = true;
            ...
            table.classList.add('quick_search_resize');
            ...
            const resizeHeader = e => {
                ...
                if (isColResizing) {
                  ...
                  var _resizing = _thead.find('th.ks_resizing .quick_search_resizer');
                  if (_resizing.length) {
            ...
            const stopResize = ev => {
                ...
                table.classList.remove('quick_search_resize');
      ...
      _thead.find(".bg-primary th").each(function() {
        ...
        if (!($(this).text() === "S.No" || $(this).hasClass("o_list_record_selector"))){
            ...
            resizeHandle.classList = 'quick_search_resizer';
            $(resizeHandle).css({
                'position': 'absolute',
                'top': '0px',
                'right': '-3px',

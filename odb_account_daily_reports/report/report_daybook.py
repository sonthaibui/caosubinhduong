...
class ReportDayBook(models.AbstractModel):
    _name = 'report.odb_account_daily_reports.report_daybook'
    ...
    def _get_account_move_entry(self, accounts, form_data, date):
        ...
        where_params = (tuple(accounts.ids), tuple(
            form_data['journal_ids']), date)
    ...
    def _get_report_values(self, docids, data=None):
        ...
        date_to = datetime.strptime(
            str(form_data['date_to']), '%Y-%m-%d').date()
        codes = []
        ...
        for date in dates:
            ...
            accounts_res = self.with_context(data['form'].get(
                'comparison_context', {}))._get_account_move_entry(accounts, form_data, date_data)

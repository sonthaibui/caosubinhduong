...
class AccountReconciliation(models.AbstractModel):
    ...
    def _get_bank_statement_line_partners(self, st_lines):
        ...
        from_clause, where_clause, where_clause_params = ir_rules_query.get_sql()
        if where_clause:
            where_partner = ('AND %s' % where_clause).replace(
                'res_partner', 'p3')
            params += where_clause_params
        else:
            where_partner = ''
    ...
    def get_bank_statement_line_data(self, st_line_ids, excluded_ids=None):
        ...
        for line in bank_statement_lines:
            # if matching_amls.get('matched_candidates_values'):
            #     results['matched_candidates_values'] =  matching_amls.get('matched_candidates_values')
    ...
    def _get_statement_line(self, st_line):
        ...
        if st_line.amount_currency and st_line.currency_id:
            ...
            amount_currency_str = formatLang(self.env, abs(
                amount_currency), currency_obj=statement_currency)
        else:
            amount = st_line.amount
            amount_currency = amount
            amount_currency_str = ""
        amount_str = formatLang(self.env, abs(
            amount), currency_obj=st_line.currency_id or statement_currency)

        data = {
            'id': st_line.id,
            'ref': st_line.ref,
            # 'note': st_line.narration or "",
            'note': html2plaintext(st_line.narration) if st_line.narration else "",
            'name': st_line.name + ": " + st_line.payment_ref,

...
class AccountReportManager(models.Model):
    _name = 'report.accounting.manager'
    ...
    summary = fields.Char()
    footnotes_ids = fields.One2many('report.accounting.footnote', 'manager_id')
    company_id = fields.Many2one('res.company')
    financial_report_id = fields.Many2one('report.html.accounting')

    def add_footnote(self, text, line):
        return self.env['report.accounting.footnote'].create({'line': line, 'text': text, 'manager_id': self.id})

class AccountReportFootnote(models.Model):
    _name = 'report.accounting.footnote'
    _description = 'Account Report Footnote'

    text = fields.Char()
    line = fields.Char(index=True)
    manager_id = fields.Many2one('report.accounting.manager')
...
class AccountReport(models.AbstractModel):
    ...
    def _get_templates(self):
        return {
                'main_template': 'odb_account_analytic_report.main_template',
                'main_table_header_template': 'odb_account_analytic_report.main_table_header',
                'line_template': 'odb_account_analytic_report.line_template',
                'footnotes_template': 'odb_account_analytic_report.footnotes_template',
                'search_template': 'odb_account_analytic_report.search_template',
                'line_caret_options': 'odb_account_analytic_report.line_caret_options',
        }
    ...
    def execute_action(self, options, params=None):
        ...
        if params.get('id'):
            ...
            if isinstance(params['id'], int):
                ...
                # It can also be a generic report.accounting id, as defined by _get_generic_line_id
    ...
    def open_tax_report_line(self, options, params=None):
        active_id = self._parse_line_id(params.get('id'))[-1][2]
        line = self.env['report.html.accounting.line'].browse(active_id)
    ...
    def open_general_ledger(self, options, params=None):
        ...
        action_vals = self.env['ir.actions.actions']._for_xml_id('odb_account_analytic_report.action_account_report_general_ledger')
        action_vals['params'] = {
            'options': options,
            'ignore_session': 'read',
        }
        ...
        if options:
            ...
            if params.get('financial_group_line_id'):
                ...
                if not (isinstance(params['financial_group_line_id'], str) and 'hierarchy_' in params['financial_group_line_id']):
                    parent_financial_report_line = self.env['report.html.accounting.line'].browse(params['financial_group_line_id'])
    ...
    def get_report_informations(self, options):
        ...
        info = {'options': options,
                ...
                'searchview_html': self.env['ir.ui.view']._render_template(self._get_templates().get('search_template', 'odb_account_analytic_report.search_template'), values=searchview_dict),
                }
        return info
    ...
    def get_html_footnotes(self, footnotes):
        template = self._get_templates().get('footnotes_template', 'odb_account_analytic_report.footnotes_template')
    ...
    #     new_wizard = self.with_context(new_context).env['odb_account_analytic_report.export.wizard'].create({'report_model': self._name, 'report_id': getattr(self, 'id', 0)})
    #     view_id = self.env.ref('odb_account_analytic_report.view_report_export_wizard').id
    #     return {
    #         'type': 'ir.actions.act_window',
    #         'name': _('Export'),
    #         'view_mode': 'form',
    #         'res_model': 'odb_account_analytic_report.export.wizard',
    #         'target': 'new',
    ...
    def _get_report_manager(self, options):
        ...
        existing_manager = self.env['report.accounting.manager'].search(domain, limit=1)
        if not existing_manager:
            existing_manager = self.env['report.accounting.manager'].create({
    ...
    def _replace_class(self):
        ...
        """
        return {
            'o_odb_account_analytic_report_no_print': '',
            'table-responsive': '',
    ...
    def get_pdf(self, options):
        ...
        body = self.env['ir.ui.view']._render_template(
            "odb_account_analytic_report.print_template",

...
class AccountingReport(models.TransientModel):
    ...
    def check_report(self):
        ...
        data = {}
        data['form'] = self.read(['account_report_id', 'date_from_cmp',
                                 'date_to_cmp', 'journal_ids', 'filter_cmp', 'target_move', 'account_analytic_ids'])[0]
    ...
    def _print_report(self, data):
        data['form'].update(self.read(['date_from_cmp', 'debit_credit', 'date_to_cmp', 'filter_cmp',
                            'account_report_id', 'enable_filter', 'label_filter', 'target_move', 'account_analytic_ids'])[0])
        return self.env.ref('odb_account_reports_pdf.action_report_financial').report_action(self, data=data, config=False)

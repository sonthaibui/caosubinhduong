...
class ReportFinancial(models.AbstractModel):
    _name = 'report.odb_account_reports_pdf.report_cash_flow'
            ...
            if report.type == 'accounts':
                ...
                for key, value in res2.items():
                    cash_in_operation = self.env.ref(
                        'odb_account_reports_pdf.cash_in_from_operation0')
                    cash_out_operation = self.env.ref(
                        'odb_account_reports_pdf.cash_out_operation1')
                    cash_in_financial = self.env.ref(
                        'odb_account_reports_pdf.cash_in_financial0')
                    cash_out_financial = self.env.ref(
                        'odb_account_reports_pdf.cash_out_financial1')
                    cash_in_investing = self.env.ref(
                        'odb_account_reports_pdf.cash_in_investing0')
                    cash_out_investing = self.env.ref(
                        'odb_account_reports_pdf.cash_out_investing1')

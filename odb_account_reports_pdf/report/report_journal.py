...
class ReportJournal(models.AbstractModel):
    _name = 'report.odb_account_reports_pdf.report_journal'
    ...
    def lines(self, target_move, journal_ids, sort_selection, data):
        ...
        query = 'SELECT "account_move_line".id FROM ' + \
            query_get_clause[0] + ', account_move am, account_account acc WHERE "account_move_line".account_id = acc.id AND "account_move_line".move_id=am.id AND am.state IN %s AND "account_move_line".journal_id IN %s AND ' + analytic_sql + query_get_clause[1] + ' ORDER BY '
    ...
    def _sum_debit(self, data, journal_id):
        ...
        query_get_clause = self._get_query_get_clause(data)
        params = [tuple(move_state), tuple(journal_id.ids)] + \
            query_get_clause[2]
        analytic_account_ids = [int(x) for x in data['form']['account_analytic_ids']]
        if len(analytic_account_ids) > 0:
            analytic_account_ids.append(0)
        ...
        self.env.cr.execute('SELECT SUM(debit) FROM ' + query_get_clause[0] + ', account_move am '
                            ...
                            query_get_clause[1] + ' '+ analytic_sql + ' ',
                            tuple(params))

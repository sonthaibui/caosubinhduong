...
class ResPartner(models.Model):
    ...
    def do_partner_manual_action_dermanord(self, followup_line_ids):
        action_text = followup_line_ids.manual_action_note or ''
        ...
        if self.payment_responsible_id:
            ...
            p = followup_line_ids.manual_action_responsible_id
    ...
    def do_partner_print(self, wizard_partner_ids, data):
        ...
        datas = {
            ...
            'model': 'account.followup',
            'form': data
        }
        return self.env.ref(
            'odb_account_followup.action_report_followup').report_action(
            self, data=datas)

    def do_partner_mail(self):
        ctx = self.env.context.copy()
        ctx['followup'] = True
        template = 'odb_account_followup.email_template_odb_account_followup_default'
        unknown_mails = 0
        for partner in self:
    ...
    def get_followup_table_html(self):
        ...
        if partner.unreconciled_aml_ids:
            ...
            report = self.env['report.odb_account_followup.report_followup']
    ...
    def do_button_print(self):
        ...
        followup_ids = self.env['account.followup'].search(
            [('company_id', '=', company_id)])
        if not followup_ids:

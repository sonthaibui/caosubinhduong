...
class FollowupPrint(models.TransientModel):
    ...
    def _get_followup(self):
        if self.env.context.get('active_model',
                                'ir.ui.menu') == 'account.followup':
            return self.env.context.get('active_id', False)
        company_id = self.env.user.company_id.id
        followp_id = self.env['account.followup'].search(
    ...
    followup_id = fields.Many2one('account.followup', 'Follow-Up',
    ...
    def do_process(self):
        ...
        resource_id = self.env.ref(
            'odb_account_followup.view_odb_account_followup_sending_results')
